/*
  ...
  - Ejercicio desde: https://2021.adventjs.dev/challenges/17
...
export function countPackages(carriers, carrierID) {
  let arrayCarriers = [carrierID];
  let total = 0;
  let i = 0;

  while (arrayCarriers.length > 0 && i < arrayCarriers.length) {
    for (const [nameId, nPackages, coleagues] of carriers) {
      let carrier = arrayCarriers[i];
      if (!carrier) continue;
      if (carrier === nameId) {
        total += nPackages;
        arrayCarriers.push(...coleagues);
      }
    }
    i++;
  }
