/*
  ...
  - Ejercicio desde: https://2021.adventjs.dev/challenges/19
...
export function learn(time, courses) {
  ...
  for (let i = 0; i < courses.length; i++) {
    for (let j = 0; j < courses.length; j++) {
      if (i === j) continue;
      let mayorSum = courses[i] + courses[j];
      if (mayorSum > mayor && mayorSum <= time) {
        mayor = mayorSum;
        arrayPos[0] = i;
        arrayPos[1] = j;
      }
  ...
  return !arrayPos[0] && !arrayPos[1] ? null : arrayPos;
}
